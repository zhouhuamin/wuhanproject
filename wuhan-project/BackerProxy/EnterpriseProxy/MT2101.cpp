...
CMT2101::CMT2101(void) : m_chOpFlag('F'), m_chSEND_FLAG('1'), m_strNote(""),m_mt2010(),m_mtResponse(),m_nFlag(0)
{
}
	...
	try
	{
		...
		if(xmlRootElement)
		{
			if ((pHeadNode = xmlRootElement->FirstChild("Head")) != NULL)
			{
				...
				m_mt2010.Head.ReceiverID	= RECEIVE;
...
{
	return 1;
	if (m_mt2010.Head.MessageID.empty())
		return 0;
...
void CMT2101::SendResponse(const std::string &strPreFlag)
{
	LOG2("CMT2101::SendResponse:%s--%s", m_mtResponse.Declaration.BorderTransportMeans.JourneyID.c_str(), strPreFlag.c_str());
	//if (m_messageHead.MESSAGE_ID[0] == '\0')
	//	return;
...
int CMT2101::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB-CUSTOM' and INOROUTFLAG='1'", m_mt2010.Head.ReceiverID.c_str());
...
int CMT2101::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='CUSTOM' and INOROUTFLAG='1'", m_mt2010.Head.ReceiverID.c_str());
...
int CMT2101::SelectMQForQiYe(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='%s' and INOROUTFLAG='1'", m_mt2010.Head.SenderID.c_str(), m_mt2010.Head.ReceiverID.c_str());
