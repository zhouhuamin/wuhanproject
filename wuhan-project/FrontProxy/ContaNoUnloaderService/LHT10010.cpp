...
CLHT10010::CLHT10010(void) : m_messageHead(), m_lht10010(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote("")
{
}

CLHT10010::~CLHT10010(void)
{
}

void CLHT10010::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CLHT10010::SaveFileName(const std::string &strFileName)
{
	...
	try
	{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				xmlSubElement = pNode->ToElement() ;
				if (xmlSubElement != NULL)
					pNodeTmp = xmlSubElement->FirstChildElement("START_DATE");
				if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
					m_lht10010.START_DATE = pNodeTmp->ToElement()->GetText();

				if (xmlSubElement != NULL)
					pNodeTmp = xmlSubElement->FirstChildElement("END_DATE");
				if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
					m_lht10010.END_DATE = pNodeTmp->ToElement()->GetText();


				xmlListElement = pNode->ToElement();
				for (pNode = xmlListElement->FirstChild("LHT_INFO"); pNode != NULL; pNode = pNode->NextSibling("LHT_INFO"))
				{
					structLHT_INFO lhtInfo;

					xmlSubElement = pNode->ToElement() ;
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("COMPANY_NAME");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						lhtInfo.COMPANY_NAME = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("CONTA_NUMBER");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						lhtInfo.CONTA_NUMBER = pNodeTmp->ToElement()->GetText();
					m_lht10010.LHT_INFO.push_back(lhtInfo);
				}
			}
		}
...
void CLHT10010::SendResponse()
{
	LOG("Enter CLHT10010::SendResponse");
	...
	for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
	{
		...
		std::string szLabel			= CMessageType::LHT10010;
...
std::string CLHT10010::GetFileName()
{
	return m_strFileName;
}

int CLHT10010::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where ENTERPRISE_CODE='LHT' and INOROUTFLAG=2");
