...
CCLM20023::CCLM20023(void) : m_messageHead(), m_clm20013(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM20023::~CCLM20023(void)
{
}

void CCLM20023::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM20023::SaveFileName(const std::string &strFileName)
{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("RESPOND_INFO")) != NULL)
				{
					...
					memcpy(m_clm20013.OP_FLAG, OP_FLAG.c_str(), OP_FLAG.size() < sizeof(m_clm20013.OP_FLAG) ? OP_FLAG.size() : sizeof(m_clm20013.OP_FLAG) - 1);
					memcpy(m_clm20013.NOTE, NOTE.c_str(), NOTE.size());
				}
...
int CCLM20023::SaveToDB()
{
	LOG("Enter CCLM20023::SaveToDB");
	...
	try
	{
		...
		if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "update D_SHIPMENT_APPLY_SHIPINFO set AUDIT_FLAG = '%c', AUDIT_DATE=GetDate(), AUDIT_NOTE='%s' where MESSAGE_ID='%s'", \
OP_FLAG,
m_clm20013.NOTE,
m_messageHead.MESSAGE_ID);
			...
			LOG1("update D_SHIPMENT_APPLY_SHIPINFO, MESSAGE_ID=%s", m_messageHead.MESSAGE_ID);
		}
	}
	catch(...)
	{
...
void CCLM20023::SendResponse()
{
	...
	std::string MESSAGE_TYPE = CMessageType::CLM20024;
...
std::string CCLM20023::GetFileName()
{
	return m_strFileName;
}

int CCLM20023::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
	...
	{
		CString strMQPath = "";
		std::string MQMessageType = CMessageType::CLM20023;
		char szSql[1024] = {0};
...
int CCLM20023::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
void CCLM20023::UTF8ToGBK( char *&szOut )
{
