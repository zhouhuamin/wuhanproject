...
CCLM10031::CCLM10031(void) : m_messageHead(),m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0),m_shipInfo(),m_billInfo(0)
{
}

CCLM10031::~CCLM10031(void)
{
}

void CCLM10031::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM10031::SaveFileName(const std::string &strFileName)
{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("SHIP_INFO")) != NULL)
				{
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("LOADING_LOCATION_ID");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						m_shipInfo.LOADING_LOCATION_ID = pNodeTmp->ToElement()->GetText();
...
int CCLM10031::SaveToDB()
{
	LOG("Enter CCLM10031::SaveToDB");
	...
	try
	{
		...
		//for (size_t i = 0; i < m_billInfo.size(); ++i)
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_SHIPMENT_DICTATE_BILL(MESSAGE_ID,CUSTOMS_CODE,I_E_FLAG,SHIP_NAME_EN,VOYAGE_NO,NOTE, \
														FUNCTION_CODE, MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME, BILL_NO, LOADING_LOCATION_ID,SHIP_ID) \
														values('%s', '%s', '%c', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%s', '%s', GetDate(), '%s', '%s', '%s')", \
														...
														m_messageHead.FUNCTION_CODE, m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE, m_messageHead.MESSAGE_CREATE_TIME, BILL_NO.c_str(), m_shipInfo.LOADING_LOCATION_ID.c_str(), m_shipInfo.SHIP_ID.c_str()); // m_billInfo[i].BILL_NO.c_str());
		...
		for (size_t i = 0; i < m_billInfo.size(); ++i)
		{
			...
			for (size_t j = 0; j < nContaSize; ++j)
			{
				...
				sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_SHIPMENT_DICTATE_CONTA(MESSAGE_ID,CONTA_NO,CONTA_MODEL,CONTA_SIZE,NOTE, BILL_NO) \
															values('%s', '%s', '%s', '%s', '%s', '%s')", \
															...
															m_billInfo[i].CONTAINER_INFO[iTmpIndex].CONTA_SIZE.c_str(), m_billInfo[i].CONTAINER_INFO[iTmpIndex].NOTE.c_str(), m_billInfo[i].BILL_NO.c_str());
...
std::string CCLM10031::GetFileName()
{
	return m_strFileName;
}

int CCLM10031::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
...
void CCLM10031::UTF8ToGBK( char *&szOut )
{
...
int CCLM10031::SelectMessageID(std::vector<std::string> &IDVect)
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
	...
	try
	{
		...
		sprintf_s(szSql, sizeof(szSql), "select MESSAGE_ID from D_SHIPMENT_DICTATE_BILL where CUSTOMS_CODE='%s' and SHIP_NAME_EN='%s' and VOYAGE_NO='%s' and RECEIVER='%s'", \
...
int CCLM10031::DeleteByID(std::vector<std::string> &IDVect)
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
	...
	try
	{
		...
		{
			CString strSql = "";
			sprintf_s(szSql, sizeof(szSql), "delete from D_SHIPMENT_DICTATE_CONTA where MESSAGE_ID='%s'",  IDVect[i].c_str());
			strSql = szSql;
			database.RAExecuteNoRs((LPCTSTR)strSql);
		}
		...
		{
			CString strSql = "";
			sprintf_s(szSql, sizeof(szSql), "delete from D_SHIPMENT_DICTATE_BILL where MESSAGE_ID='%s'",  IDVect[i].c_str());
			strSql = szSql;
			database.RAExecuteNoRs((LPCTSTR)strSql);
		}
		database.GetConn()->CommitTrans();
