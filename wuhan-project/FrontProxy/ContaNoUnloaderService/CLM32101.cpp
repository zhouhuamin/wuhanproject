...
CCLM32101::CCLM32101(void) : m_messageHead(), m_lht10010(), m_chOpFlag('F'), \
...
{
	LOG3("Enter CCLM32101::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	...
	for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
	{
		...
		std::string szLabel			= CMessageType::CLM32101;
		szLabel += "-";
