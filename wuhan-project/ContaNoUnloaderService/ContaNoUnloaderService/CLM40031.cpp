		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("BILL_INFO")) != NULL)
				{
					...
					memcpy(m_billInfo.SHIP_ID, SHIP_ID.c_str(), SHIP_ID.size());
...
FUNCTION_CODE, MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME,MOVE_ARR_TIME,SHIP_ID) \
values('%s', '%s', '%c', '%s', '%s', '%s', '%s', \
'%s', '%s', '%s', '%s', '%s', GetDate(),'%s','%s')", \
...
void CCLM40031::UTF8ToGBK( char *&szOut )
{
