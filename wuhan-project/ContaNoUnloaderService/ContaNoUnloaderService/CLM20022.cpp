	...
	try
	{
		...
		if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_CUSTOMREPORT_RESPONSE(MESSAGE_ID,FUNCTION_CODE,MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME, OP_FLAG, NOTE, VERSION) \
														...
														//OP_FLAG,
														//NOTE,
														m_respondInfo.OP_FLAG,
														m_respondInfo.NOTE,
														m_messageHead.VERSION);
			...
			m_chOpFlag = 'T';
			LOG1("insert into D_CUSTOMREPORT_RESPONSE, MESSAGE_ID=%s", m_messageHead.MESSAGE_ID);
		}
	}
	catch(...)
	{
...
int CCLM20022::SaveBLOB()
{
	try
	{
		...
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select * from D_CUSTOMREPORT_RESPONSE where MESSAGE_ID='%s'", m_messageHead.MESSAGE_ID);
...
void CCLM20022::UTF8ToGBK( char *&szOut )
{
