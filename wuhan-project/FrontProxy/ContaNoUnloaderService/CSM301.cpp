...
{
	LOG3("Enter CCSM301::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_csm301.MessageHead.MessageType.c_str(), m_csm301.MessageHead.SenderID.c_str(), m_csm301.MessageHead.ReceiverID.c_str());
	...
	for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
	{
		...
		szLabel += "-";
		szLabel += m_csm301.MessageHead.MessageID;
...
int CCSM301::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	...
	try
	{
		...
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where ENTERPRISE_CODE='%s' and INOROUTFLAG=1", m_csm301.MessageHead.ReceiverID.c_str());
