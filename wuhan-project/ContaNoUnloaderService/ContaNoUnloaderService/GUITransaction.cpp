...
{
	std::vector<structDataItem> guidVect;
	...
	for (size_t i = 0; i < guidVect.size(); ++i)
	{
		...
		std::string strGUID		   = guidVect[i].strGUID;
		std::string strTime		   = guidVect[i].strTime;
		...
		GetXmlFromDB(strGUID, strMessageType, strSender, spXmlData, nXmlLen, strTime);
...
int CGUITransaction::GetXmlFromDB(const std::string &strParaGUID, std::string &strMessageType,std::string &strSender,boost::shared_array<char> &spXmlData, int &nXmlLen, const std::string &strTime)
{
	try
	{
		char szSql[512] = {0};
		sprintf_s(szSql, sizeof(szSql), "select top 1 GUID, MESSAGE_TYPE, SENDER, REPORT from R_APPLY_REPORT where GUID = '%s' and DEL_FLAG='0'", strParaGUID.c_str());
		CString strResult		= "";
		CString strConn			= "";
		CString strGUID			= "";
...
int CGUITransaction::UpdateDB(const std::string &strGUID, const std::string &strTime)
{
	//if (m_strMQIp.empty() || m_strQueueName.empty())
	//	return 0;
	...
	try
	{
		...
		if (!strGUID.empty())
		{
			CString strSql = "";
			char szSqlBuffer[2048] = {0};
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "update R_APPLY_REPORT set DEL_FLAG='%c' where GUID='%s' and DEL_FLAG='0'",  '1', strGUID.c_str());
...
int CGUITransaction::GetGUIDFromDB(std::vector<structDataItem> &guidVect)
{
	try
	{
		char szSql[512] = {0};
		sprintf_s(szSql, sizeof(szSql), "select top 5 GUID, MESSAGE_CREATE_TIME from R_APPLY_REPORT where DEL_FLAG = '0'");
		CString strResult		= "";
		CString strConn			= "";
		std::string strGUID			= "";
		std::string MESSAGE_CREATE_TIME	= "";
		CString MESSAGE_TYPE	= "";
		CString SENDER			= "";
		strConn = "Provider=SQLOLEDB.1;Password="; //sa;Persist Security Info=True;User ID=sa;Initial Catalog=CGS_DB;Data Source=SANBAO-ZHOUHM
		//strConn += "sa";
		...
		while (!oRs->EndOfFile)
		{	
			structDataItem tmpItem;
			val = oRs->Fields->Item[_variant_t("GUID")]->Value;
			if (val.vt != (VT_NULL))
			{
				tmpItem.strGUID = strGUID = (char*)_bstr_t(val);	
				
			}

			val = oRs->Fields->Item[_variant_t("MESSAGE_CREATE_TIME")]->Value;
			if (val.vt != (VT_NULL))
			{
				tmpItem.strTime = MESSAGE_CREATE_TIME = (char*)_bstr_t(val);	
				guidVect.push_back(tmpItem);
			}
