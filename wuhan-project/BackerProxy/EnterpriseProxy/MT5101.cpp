...
void CMT5101::SendResponse(const std::string &strPreFlag)
{
	LOG2("CMT5101::SendResponse:%s--%s", m_mtResponse.Declaration.BorderTransportMeans.JourneyID.c_str(), strPreFlag.c_str());
	//if (m_messageHead.MESSAGE_ID[0] == '\0')
	//	return;
...
int CMT5101::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray, char INOROUTFLAG)
{
	try
	{
		CStringArray mqPathArray;
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='CUSTOM' and INOROUTFLAG='%c'", m_mt5101.Head.ReceiverID.c_str(), INOROUTFLAG);
...
int CMT5101::SelectMQForQiYe(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
