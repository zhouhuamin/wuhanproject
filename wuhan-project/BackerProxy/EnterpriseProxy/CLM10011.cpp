		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("BILL_INFO")) != NULL)
				{
					...
					// 赋值
					m_billInfo.CUSTOMS_CODE = CUSTOMS_CODE;
					m_billInfo.I_E_FLAG = I_E_FLAG;
					m_billInfo.SHIP_NAME_EN = SHIP_NAME_EN;
					m_billInfo.VOYAGE_NO = VOYAGE_NO;
					m_billInfo.BILL_NO = BILL_NO;
					m_billInfo.NOTE = NOTE;
...
void CCLM10011::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM10011::SendResponse:%s", strPreFlag.c_str());
	...
	//	std::string szLabel			= CMessageType::CLM10011;
...
int CCLM10011::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
