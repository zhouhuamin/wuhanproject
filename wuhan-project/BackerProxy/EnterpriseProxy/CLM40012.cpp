...
CCLM40012::CCLM40012(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM40012::~CCLM40012(void)
{
}

void CCLM40012::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM40012::SaveFileName(const std::string &strFileName)
{
...
void CCLM40012::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM40012::SendResponse:%s", strPreFlag.c_str());
	// 海关平台返回的回执
	if (strPreFlag == "WEB")
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			std::string szLabel			= CMessageType::CLM40012;
...
std::string CCLM40012::GetFileName()
{
	return m_strFileName;
}

int CCLM40012::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM40012::SelectResponseInfo(CString &strMESSAGE_ID, CString &strOP_FLAG, CString &strNOTE)
{
...
int CCLM40012::InsertResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM40012::DeleteResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM40012::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
