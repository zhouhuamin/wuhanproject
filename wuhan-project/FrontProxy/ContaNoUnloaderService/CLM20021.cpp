...
CCLM20021::CCLM20021(void) : m_messageHead(), m_shipInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_billInfo(0)
{
}

CCLM20021::~CCLM20021(void)
{
}

void CCLM20021::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM20021::SaveFileName(const std::string &strFileName)
{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("SHIP_INFO")) != NULL)
				{
					...
					string NOTE = "";
					string CHANGE_SHIP_NAME_EN	= "";
					string CHANGE_SHIP_NAME_CN	= "";
					string CHANGE_VOYAGE_NO		= "";
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("CHANGE_SHIP_NAME_EN");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						CHANGE_SHIP_NAME_EN = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("CHANGE_SHIP_NAME_CN");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						CHANGE_SHIP_NAME_CN = pNodeTmp->ToElement()->GetText();


					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("CHANGE_VOYAGE_NO");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						CHANGE_VOYAGE_NO = pNodeTmp->ToElement()->GetText();
					...
					//// 赋值
					//memcpy(m_shipInfo.CUSTOMS_CODE, CUSTOMS_CODE.c_str(), sizeof(m_shipInfo.CUSTOMS_CODE) - 1);
					//memcpy(m_shipInfo.I_E_FLAG, I_E_FLAG.c_str(), sizeof(m_shipInfo.I_E_FLAG) - 1);
					//memcpy(m_shipInfo.NATIVE_SHIP_NAME_EN, NATIVE_SHIP_NAME_EN.c_str(), sizeof(m_shipInfo.NATIVE_SHIP_NAME_EN) - 1);
					//memcpy(m_shipInfo.NATIVE_SHIP_NAME_CN, NATIVE_SHIP_NAME_CN.c_str(), sizeof(m_shipInfo.NATIVE_SHIP_NAME_CN) - 1);
					//memcpy(m_shipInfo.NATIVE_VOYAGE_NO, NATIVE_VOYAGE_NO.c_str(), sizeof(m_shipInfo.NATIVE_VOYAGE_NO) - 1);

					//memcpy(m_shipInfo.CHANGE_SHIP_NAME_EN, CHANGE_SHIP_NAME_EN.c_str(), sizeof(m_shipInfo.CHANGE_SHIP_NAME_EN) - 1);
					//memcpy(m_shipInfo.CHANGE_SHIP_NAME_CN, CHANGE_SHIP_NAME_CN.c_str(), sizeof(m_shipInfo.CHANGE_SHIP_NAME_CN) - 1);
					//memcpy(m_shipInfo.CHANGE_VOYAGE_NO, CHANGE_VOYAGE_NO.c_str(), sizeof(m_shipInfo.CHANGE_VOYAGE_NO) - 1);


					//memcpy(m_shipInfo.START_CUSTOMS_CODE, START_CUSTOMS_CODE.c_str(), sizeof(m_shipInfo.START_CUSTOMS_CODE) - 1);
					//memcpy(m_shipInfo.END_CUSTOMS_CODE, END_CUSTOMS_CODE.c_str(), sizeof(m_shipInfo.END_CUSTOMS_CODE) - 1);
					//memcpy(m_shipInfo.IMO, IMO.c_str(), sizeof(m_shipInfo.IMO) - 1);
					//memcpy(m_shipInfo.SHIP_ID, SHIP_ID.c_str(), sizeof(m_shipInfo.SHIP_ID) - 1);


					//memcpy(m_shipInfo.NOTE, NOTE.c_str(), sizeof(m_shipInfo.NOTE) - 1);
				}
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				{
					structBILL_INFO_CLM20021 clm20021;
					string BILL_NO		= "";
					string CONTA_MODEL	= "";
					string CONTA_SIZE	= "";
					string NOTE			= "";
					...
					if (xmlBILL_INFOElement != NULL)
					{
						...
						{
							structCONTAINER_INFO_CLM20021 tmpContainerInfo;
							...
							clm20021.CONTAINER_INFO.push_back(tmpContainerInfo);
						}
					}
...
int CCLM20021::SaveToDB()
{
	...
	try
	{
		...
		for (size_t i = 0; i < nBillInfoSize; ++i)
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_SHIPMENT_APPLY_BILL(MESSAGE_ID,BILL_NO,NOTE) values('%s', '%s', '%s')", \
		...
		if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			CString strSql = "";
			char szSqlBuffer[4096] = {0};
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_SHIPMENT_APPLY_SHIPINFO(MESSAGE_ID,CUSTOMS_CODE,I_E_FLAG,SHIP_NAME_EN,NATIVE_SHIP_NAME_CN,\
START_CUSTOMS_CODE,END_CUSTOMS_CODE,IMO,SHIP_ID,VOYAGE_NO,NOTE, \
FUNCTION_CODE, MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME, SEND_FLAG, CHANGE_SHIP_NAME_EN,CHANGE_SHIP_NAME_CN,CHANGE_VOYAGE_NO) \
values('%s', '%s', '%c', '%s', '%s',\
'%s', '%s', '%s', '%s', '%s', '%s', \
'%s', '%s', '%s', '%s', '%s', GetDate(), '%c','%s','%s','%s')", \
m_messageHead.MESSAGE_ID, 
...
SEND_FLAG,
m_shipInfo.CHANGE_SHIP_NAME_EN,
m_shipInfo.CHANGE_SHIP_NAME_CN,
m_shipInfo.CHANGE_VOYAGE_NO
);
		...
		for (size_t i = 0; i < nBillInfoSize; ++i)
		{
			...
			for (size_t j = 0; j < nContainerInfoSize; ++j)
			{
				...
				CString strSql = "";
				char szSqlBuffer[1024] = {0};
				sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_SHIPMENT_APPLY_CONTA(MESSAGE_ID,CONTA_NO,CONTA_MODEL,CONTA_SIZE,BILL_NO, NOTE) \
values('%s', '%s', '%s', '%s', '%s', '%s')", \
m_messageHead.MESSAGE_ID,
...
void CCLM20021::SendResponse()
{
	LOG3("Enter CCLM20021::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	...
	std::string szLabel			= m_messageHead.MESSAGE_TYPE; // CMessageType::CLM20021;
...
//	CString strResponse = "";
//
//	std::string MESSAGE_TYPE = CMessageType::CLM20022;
//
...
//	apBasic->SendMessage((LPCTSTR)strMessage, strMessage.GetLength());
	...
	//	std::string strQueueName	= (LPCTSTR)queueNameArray[j];
	//	std::string strMQIp			= (LPCTSTR)mqIPArray[i];
	//	std::string szLabel			= CMessageType::CLM20021;
	//	std::auto_ptr<CMessageSender> apBasic(new CMSMQSender(strMQIp, strQueueName, szLabel, szLabel));
	//	apBasic->SendMessage(spXmlData.get(), iLen);
	//}
...
std::string CCLM20021::GetFileName()
{
	return m_strFileName;
}

int CCLM20021::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM20021::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
