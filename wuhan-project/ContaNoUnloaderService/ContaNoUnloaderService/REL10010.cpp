...
void CREL10010::ParseMessage()
{
	...
	{	
		...
		{
			std::string strAGENT_NAME = "";
			strAGENT_NAME = AGENT_NAMEHandle.Text()->Value();

			if (strAGENT_NAME != "")
			{
				char szNote[256] = {0};
				char *pTmp = szNote;
				memcpy(szNote, strAGENT_NAME.c_str(), strAGENT_NAME.size() < 256 ? strAGENT_NAME.size() : 255);
				UTF8ToGBK(pTmp);
				AGENT_NAME = string(szNote);
			}
			else
			{
...
int CREL10010::SaveToDB()
{
	...
	try
	{
		...
		//if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into H_AIR_ENTRY_LIST(ENTRY_ID,TRAF_NAME,VOYAGE_NO,BILL_NO,AGENT_NAME,PACK_NO,GROSS_WT,INPUT_TIME) \
...
dGROSS_WT
			...
			LOG1("insert into H_AIR_ENTRY_LIST, ENTRY_ID=%s", ENTRY_ID.c_str());
		}
	}
	catch(...)
	{
...
void CREL10010::UTF8ToGBK( char *&szOut )
{
