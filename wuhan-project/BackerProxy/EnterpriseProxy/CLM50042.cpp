...
CCLM50042::CCLM50042(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM50042::~CCLM50042(void)
{
}

void CCLM50042::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM50042::SaveFileName(const std::string &strFileName)
{
...
void CCLM50042::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM50042::SendResponse:%s", strPreFlag.c_str());
	// 海关平台返回的回执
	if (strPreFlag == "WEB")
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			std::string szLabel			= CMessageType::CLM50042;
...
std::string CCLM50042::GetFileName()
{
	return m_strFileName;
}

int CCLM50042::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM50042::SelectResponseInfo(CString &strMESSAGE_ID, CString &strOP_FLAG, CString &strNOTE)
{
...
int CCLM50042::InsertResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM50042::DeleteResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM50042::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
