...
CCLM60011::CCLM60011(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM60011::~CCLM60011(void)
{
}

void CCLM60011::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM60011::SaveFileName(const std::string &strFileName)
{
...
int CCLM60011::SaveToDB()
{
	...
	try
	{
		...
		if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_UNLOADCONTA_BILL(MESSAGE_ID,CUSTOMS_CODE,I_E_FLAG,SHIP_NAME_EN,VOYAGE_NO,NOTE, \
...
m_messageHead.FUNCTION_CODE, m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE, m_messageHead.MESSAGE_CREATE_TIME);
		...
		for (size_t i = 0; i < nContaSize; ++i)
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_UNLOADCONTA_CONTA(MESSAGE_ID,CONTA_NO,CONTA_MODEL,CONTA_SIZE,NOTE) \
														values('%s', '%s', '%s', '%s', '%s')", \
...
void CCLM60011::SendResponse()
{
	LOG3("Enter CCLM60011::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	...
	std::string MESSAGE_TYPE = CMessageType::CLM60012;
...
m_messageHead.SENDER, MESSAGE_CREATE_TIME, m_messageHead.VERSION, m_messageHead.MESSAGE_ID, m_chOpFlag, m_strNote.c_str());
	...
	// 发送MSMQ
	//CMSMQApiWrapper ob;
	std::string szLabel = CMessageType::CLM60012;
	...
	//	std::string strQueueName	= (LPCTSTR)queueNameArray[j];
	//	std::string strMQIp			= (LPCTSTR)mqIPArray[i];
	//	std::string szLabel			= CMessageType::CLM60011;
	//	std::auto_ptr<CMessageSender> apBasic(new CMSMQSender(strMQIp, strQueueName, szLabel, szLabel));
	//	apBasic->SendMessage(spXmlData.get(), iLen);
	//}
...
std::string CCLM60011::GetFileName()
{
	return m_strFileName;
}

int CCLM60011::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM60011::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
