		...
		try
		{
			...
			while (msgRec != NULL)
			{
				...
				*pMessage = pData;
				...
				//CString strXmlData;
				//strXmlData.Replace("<string>", "");
				//strXmlData.Replace("</string>", "");

				{
					CString strXmlData2 = "";
					CString strXmlData(pData);
					TiXmlDocument doc;
					doc.Parse(LPCTSTR(strXmlData));
					TiXmlHandle docHandle(&doc);
					TiXmlHandle XmlDataHandle = docHandle.FirstChildElement("string").FirstChild();
					if (XmlDataHandle.Node() != NULL)		
						strXmlData2  = XmlDataHandle.Text()->Value();

					if (strXmlData2 != "")
					{
						CStdioFile  file;
						file.Open((LPCTSTR)strFileName,CFile::modeCreate | CFile::modeWrite | CFile::typeBinary);
						file.Write((LPCTSTR)strXmlData2, strXmlData2.GetLength());
						file.Close();
					}
					else
					{
