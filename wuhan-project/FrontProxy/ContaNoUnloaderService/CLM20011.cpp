		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("SHIP_INFO")) != NULL)
				{
					...
					if (I_E_FLAG != "")
					{
						m_shipInfo.I_E_FLAG[0] = I_E_FLAG[0];
					}
...
{
	LOG3("Enter CCLM20011::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
