...
CCLM90012::CCLM90012(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM90012::~CCLM90012(void)
{
}

void CCLM90012::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM90012::SaveFileName(const std::string &strFileName)
{
...
void CCLM90012::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM90012::SendResponse:%s", strPreFlag.c_str());
	// 海关平台返回的回执
	if (strPreFlag == "WEB")
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			std::string szLabel			= CMessageType::CLM90012;
...
std::string CCLM90012::GetFileName()
{
	return m_strFileName;
}

int CCLM90012::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM90012::SelectResponseInfo(CString &strMESSAGE_ID, CString &strOP_FLAG, CString &strNOTE)
{
...
int CCLM90012::InsertResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM90012::DeleteResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM90012::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
