...
void CCSM003::SendResponse(const std::string &strPreFlag)
{
	//if (m_messageHead.MESSAGE_ID[0] == '\0')
	//	return;
...
int CCSM003::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB-CUSTOM' and INOROUTFLAG='1'", m_csm003.MessageHead.ReceiverID.c_str());
...
int CCSM003::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='CUSTOM' and INOROUTFLAG='1'", m_csm003.MessageHead.ReceiverID.c_str());
