...
CCSM002::CCSM002(void) : m_chOpFlag('F'), m_chSEND_FLAG('1'), m_strNote(""),m_csm002()
{
}

CCSM002::~CCSM002(void)
{
}

void CCSM002::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCSM002::SaveFileName(const std::string &strFileName)
{
...
//
//int CCSM002::SaveLocal(BYTE *pMessage, int iLen)
//{
...
void CCSM002::ParseMessage()
{
	try
	{
		...
		if (MessageIDHandle.Node() != NULL)	
			m_csm002.MessageHead.MessageID 			= MessageIDHandle.Text()->Value();
		if (FunctionCodeHandle.Node() != NULL)	
			m_csm002.MessageHead.FunctionCode 			= FunctionCodeHandle.Text()->Value();
		if (MessageTypeHandle.Node() != NULL)	
			m_csm002.MessageHead.MessageType 			= MessageTypeHandle.Text()->Value();
		if (SenderIDHandle.Node() != NULL)	
			m_csm002.MessageHead.SenderID 			= SenderIDHandle.Text()->Value();
		if (ReceiverIDHandle.Node() != NULL)	
			m_csm002.MessageHead.ReceiverID 			= ReceiverIDHandle.Text()->Value();
		if (SendTimeHandle.Node() != NULL)	
			m_csm002.MessageHead.SendTime 			= SendTimeHandle.Text()->Value();
		if (VersionHandle.Node() != NULL)	
			m_csm002.MessageHead.Version 			= VersionHandle.Text()->Value();

		if (ID.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.ID 			= ID.Text()->Value();
		if (Name.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.Name 			= Name.Text()->Value();
		if (JourneyID.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.JourneyID 					= JourneyID.Text()->Value();
		//if (JourneyID2.Node() != NULL)	
		m_csm002.MessageBody.Declaration.BorderTransPortMeans.JourneyID2 						= JourneyID2;

		if (ArrivalDateTime.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.Itinerary.ArrivalDateTime 					= ArrivalDateTime.Text()->Value();

		if (StayID.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.StayID 					= StayID.Text()->Value();

		if (DepartureConveyanceFacilityID.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.DepartureConveyanceFacility.ID 					= DepartureConveyanceFacilityID.Text()->Value();

		if (ArrivalConveyanceFacilityID.Node() != NULL)	
			m_csm002.MessageBody.Declaration.BorderTransPortMeans.ArrivalConveyanceFacility.ID					= ArrivalConveyanceFacilityID.Text()->Value();

		if (LoadingDate.Node() != NULL)	
			m_csm002.MessageBody.Declaration.LoadingLocation.LoadingDate 					= LoadingDate.Text()->Value();

		if (AgentID.Node() != NULL)	
			m_csm002.MessageBody.Declaration.Agent.ID 					= AgentID.Text()->Value();

		if (ContactName.Node() != NULL)	
			m_csm002.MessageBody.Declaration.Agent.Contact.Name					= ContactName.Text()->Value();

		if (IssueDate.Node() != NULL)	
			m_csm002.MessageBody.Declaration.IssueDate 					= IssueDate.Text()->Value();

		if (Content.Node() != NULL)	
			m_csm002.MessageBody.Declaration.AdditionalInformation.Content 					= Content.Text()->Value();
...
int CCSM002::SaveToDB()
{
	return 1;
	if (m_csm002.MessageHead.MessageID.empty())
		return 0;
	...
	try
	{
		...
		if (!m_csm002.MessageHead.MessageID.empty())
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_IM_FORECAST(MessageID,FunctionCode,MessageType,SenderID,ReceiverID,SendTime, \
														IMO, SHIP_NAME_EN, IM_VOYAGE_NO, EX_VOYAGE_NO, ARRIVE_TIME, \
														StayID, LOADING_PORT, DESTINATION_PORT, LOADING_TIME, AGENT_CODE,\
														AGENT_NAME,ISSUE_TIME,CONTENT,I_E_FLAG) \
														values('%s', '%s', '%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%c')", \
														m_csm002.MessageHead.MessageID.c_str(),
														m_csm002.MessageHead.FunctionCode.c_str(),
														m_csm002.MessageHead.MessageType.c_str(),
														m_csm002.MessageHead.SenderID.c_str(),
														m_csm002.MessageHead.ReceiverID.c_str(),
														m_csm002.MessageHead.SendTime.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.ID.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.Name.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.JourneyID.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.JourneyID2.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.Itinerary.ArrivalDateTime.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.StayID.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.DepartureConveyanceFacility.ID.c_str(),
														m_csm002.MessageBody.Declaration.BorderTransPortMeans.ArrivalConveyanceFacility.ID.c_str(),
														m_csm002.MessageBody.Declaration.LoadingLocation.LoadingDate.c_str(),
														m_csm002.MessageBody.Declaration.Agent.ID.c_str(),
														m_csm002.MessageBody.Declaration.Agent.Contact.Name.c_str(),
														m_csm002.MessageBody.Declaration.IssueDate.c_str(),
														m_csm002.MessageBody.Declaration.AdditionalInformation.Content.c_str(),
														I_E_FLAG);
			...
			LOG1("insert into D_IM_FORECAST, MESSAGE_ID=%s", m_csm002.MessageHead.MessageID.c_str());
		}
...
void CCSM002::SendResponse()
{
	LOG3("Enter CCSM002::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_csm002.MessageHead.MessageType.c_str(), m_csm002.MessageHead.SenderID.c_str(), m_csm002.MessageHead.ReceiverID.c_str());
	if (m_csm002.MessageHead.MessageID.empty())
		return;
	...
	std::string szLabel			= CMessageType::CSM002;
	...
	//	m_csm002.MessageHead.SenderID.c_str(),
	//	MESSAGE_CREATE_TIME,
	//	m_csm002.MessageHead.Version.c_str(),
	//	m_csm002.MessageHead.MessageID.c_str(),
	//	m_csm002.MessageBody.Declaration.BorderTransPortMeans.ID.c_str(),
	//	m_csm002.MessageBody.Declaration.BorderTransPortMeans.JourneyID.c_str(),
	//	RecType.c_str(),
...
std::string CCSM002::GetFileName()
{
	return m_strFileName;
}

int CCSM002::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	if (m_csm002.MessageHead.MessageID.empty())
		return 0;
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='PROXY' and INOROUTFLAG='0'", m_csm002.MessageHead.ReceiverID.c_str());
...
int CCSM002::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='WEB' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB' and INOROUTFLAG='0'", m_csm002.MessageHead.ReceiverID.c_str());
