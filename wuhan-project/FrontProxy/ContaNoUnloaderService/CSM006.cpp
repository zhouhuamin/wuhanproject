...
CCSM006::CCSM006(void) : m_chOpFlag('F'), m_chSEND_FLAG('1'), m_strNote(""),m_csm002()
{
}

CCSM006::~CCSM006(void)
{
}

void CCSM006::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCSM006::SaveFileName(const std::string &strFileName)
{
...
//
//int CCSM006::SaveLocal(BYTE *pMessage, int iLen)
//{
...
void CCSM006::SendResponse()
{
	LOG3("Enter CCSM006::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_csm002.MessageHead.MessageType.c_str(), m_csm002.MessageHead.SenderID.c_str(), m_csm002.MessageHead.ReceiverID.c_str());
	if (m_csm002.MessageHead.MessageID.empty())
		return;
	...
	std::string szLabel			= CMessageType::CSM006;
...
std::string CCSM006::GetFileName()
{
	return m_strFileName;
}

int CCSM006::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	if (m_csm002.MessageHead.MessageID.empty())
		return 0;
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='ALL' and INOROUTFLAG='0'", m_csm002.MessageHead.ReceiverID.c_str());
...
int CCSM006::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
