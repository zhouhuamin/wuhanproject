...
CCLM60013::CCLM60013(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM60013::~CCLM60013(void)
{
}

void CCLM60013::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM60013::SaveFileName(const std::string &strFileName)
{
...
m_messageHead.MESSAGE_ID, OP_FLAG, m_messageHead.MESSAGE_CREATE_TIME, m_respondInfo.NOTE);
			...
			LOG1("insert into D_UNLOADCONTA_AUDIT, MESSAGE_ID=%s", m_messageHead.MESSAGE_ID);
		}
	}
	catch(...)
	{
...
void CCLM60013::SendResponse()
{
	LOG3("Enter CCLM60013::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return;
	...
	std::string MESSAGE_TYPE = CMessageType::CLM60014;
	...
	// 发送MSMQ
	//CMSMQApiWrapper ob;
	std::string szLabel = CMessageType::CLM60014;
...
std::string CCLM60013::GetFileName()
{
	return m_strFileName;
}

int CCLM60013::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
