...
void CCLM10051::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM10051::SendResponse:%s", strPreFlag.c_str());
	...
	for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
	{
		...
		std::string szLabel			= CMessageType::CLM10051;
...
int CCLM10051::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
