...
CCSM005::CCSM005(void) : m_chOpFlag('F'), m_chSEND_FLAG('1'), m_strNote(""),m_csm005()
{
}

CCSM005::~CCSM005(void)
{
}

void CCSM005::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCSM005::SaveFileName(const std::string &strFileName)
{
...
//
//int CCSM005::SaveLocal(BYTE *pMessage, int iLen)
//{
...
void CCSM005::ParseMessage()
{
	try
	{
		...
		pNode = docHandle.FirstChildElement("Message").FirstChildElement("MessageBody").ChildElement("Declaration", 0).ChildElement("BorderTransPortMeans", 0).ChildElement("StayID", 0).ToNode();
		std::string StayID2 = "";
		if (pNode != NULL)
		{
			pNode = pNode->NextSibling("StayID");
			if (pNode != NULL)
			{
				if (pNode->ToElement() != NULL)
					StayID2 = pNode->ToElement()->GetText();
			}
		}


		TiXmlHandle DataTime = docHandle.FirstChildElement("Message").FirstChildElement("MessageBody").ChildElement("Declaration", 0).ChildElement("BorderTransPortMeans", 0).ChildElement("DataTime", 0).FirstChild();
		...
		if (MessageIDHandle.Node() != NULL)	
			m_csm005.MessageHead.MessageID 			= MessageIDHandle.Text()->Value();
		if (FunctionCodeHandle.Node() != NULL)	
			m_csm005.MessageHead.FunctionCode 			= FunctionCodeHandle.Text()->Value();
		if (MessageTypeHandle.Node() != NULL)	
			m_csm005.MessageHead.MessageType 			= MessageTypeHandle.Text()->Value();
		if (SenderIDHandle.Node() != NULL)	
			m_csm005.MessageHead.SenderID 			= SenderIDHandle.Text()->Value();
		if (ReceiverIDHandle.Node() != NULL)	
			m_csm005.MessageHead.ReceiverID 			= ReceiverIDHandle.Text()->Value();
		if (SendTimeHandle.Node() != NULL)	
			m_csm005.MessageHead.SendTime 			= SendTimeHandle.Text()->Value();
		if (VersionHandle.Node() != NULL)	
			m_csm005.MessageHead.Version 			= VersionHandle.Text()->Value();

		if (ID.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.ID 			= ID.Text()->Value();
		if (Name.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.Name 			= Name.Text()->Value();
		if (JourneyID.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.JourneyID 					= JourneyID.Text()->Value();
		//if (JourneyID2.Node() != NULL)	
		m_csm005.MessageBody.Declaration.BorderTransPortMeans.JourneyID2 						= JourneyID2;

		if (ArrivalDateTime.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.Itinerary.ArrivalDateTime 					= ArrivalDateTime.Text()->Value();

		if (StayID.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.StayID 					= StayID.Text()->Value();
		m_csm005.MessageBody.Declaration.BorderTransPortMeans.StayID2	= StayID2;

		if (DataTime.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.DataTime 					= DataTime.Text()->Value();

		if (DepartureConveyanceFacilityID.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.DepartureConveyanceFacility.ID 					= DepartureConveyanceFacilityID.Text()->Value();

		if (ArrivalConveyanceFacilityID.Node() != NULL)	
			m_csm005.MessageBody.Declaration.BorderTransPortMeans.ArrivalConveyanceFacility.ID					= ArrivalConveyanceFacilityID.Text()->Value();



		if (AgentID.Node() != NULL)	
			m_csm005.MessageBody.Declaration.Agent.ID 					= AgentID.Text()->Value();

		if (ContactName.Node() != NULL)	
			m_csm005.MessageBody.Declaration.Agent.Contact.Name					= ContactName.Text()->Value();

		if (IssueDate.Node() != NULL)	
			m_csm005.MessageBody.Declaration.IssueDate 					= IssueDate.Text()->Value();

		if (Content.Node() != NULL)	
			m_csm005.MessageBody.Declaration.AdditionalInformation.Content 					= Content.Text()->Value();
...
int CCSM005::SaveToDB()
{
	return 1;
	if (m_csm005.MessageHead.MessageID.empty())
		return 0;
...
void CCSM005::SendResponse(const std::string &strPreFlag)
{
	//if (m_messageHead.MESSAGE_ID[0] == '\0')
	//	return;
	...
	//// 发送MSMQ
	////CMSMQApiWrapper ob;
	//std::string szLabel = CMessageType::CSM005;
...
std::string CCSM005::GetFileName()
{
	return m_strFileName;
}

int CCSM005::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	if (m_csm005.MessageHead.MessageID.empty())
		return 0;
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB-CUSTOM' and INOROUTFLAG='1'", m_csm005.MessageHead.ReceiverID.c_str());
...
int CCSM005::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='CUSTOM' and INOROUTFLAG='1'", m_csm005.MessageHead.ReceiverID.c_str());
