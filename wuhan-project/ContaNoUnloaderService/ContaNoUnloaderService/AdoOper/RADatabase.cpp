...
HRESULT CRADatabase::RAExecuteNoRs(const char *SqlStatement)
{
	...
	{		
		...
		{
			// printf("Description = %s, %s\n", SqlStatement, (char*) e.Description());  
			CString strMsg = "";
			strMsg = (char*) e.Description();
			LOG2("%s, %s", SqlStatement, strMsg);
		}
		catch(...)
		{
		}
...
_RecordsetPtr CRADatabase::RAExecuteRsForUpdate(const char *SqlStatement)
{
	// Attempt to execute a SQL statement, return the RS, and leave connection open
	if (strlen(SqlStatement) > 0)
	{		
		try
		{
			...
			m_Recordset->Open(_variant_t(SqlStatement),_variant_t((IDispatch*)m_Connection,TRUE), adOpenStatic,adLockOptimistic,adCmdText);
...
_ConnectionPtr CRADatabase::GetConn()
{
	return m_Connection; 
}
