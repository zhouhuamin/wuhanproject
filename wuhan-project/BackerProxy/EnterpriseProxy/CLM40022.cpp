...
CCLM40022::CCLM40022(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM40022::~CCLM40022(void)
{
}

void CCLM40022::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM40022::SaveFileName(const std::string &strFileName)
{
...
void CCLM40022::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM40022::SendResponse:%s", strPreFlag.c_str());
	// 海关平台返回的回执
	if (strPreFlag == "WEB")
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			std::string szLabel			= CMessageType::CLM40022;
...
std::string CCLM40022::GetFileName()
{
	return m_strFileName;
}

int CCLM40022::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM40022::SelectResponseInfo(CString &strMESSAGE_ID, CString &strOP_FLAG, CString &strNOTE)
{
...
int CCLM40022::InsertResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM40022::DeleteResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM40022::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
