	...
	try
	{
		...
		if (!m_csm007.MessageHead.MessageID.empty())
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_IM_REPORTS(MessageID,FunctionCode,MessageType,SenderID,ReceiverID,SendTime, \
														...
														'%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%c')", \
														m_csm007.MessageHead.MessageID.c_str(),
														m_csm007.MessageHead.FunctionCode.c_str(),
														m_csm007.MessageHead.MessageType.c_str(),
														m_csm007.MessageHead.SenderID.c_str(),
														m_csm007.MessageHead.ReceiverID.c_str(),
														m_csm007.MessageHead.SendTime.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.ID.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.Name.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.JourneyID.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.JourneyID2.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.Itinerary.ArrivalDateTime.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.StayID.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.DepartureConveyanceFacility.ID.c_str(),
														m_csm007.MessageBody.Declaration.BorderTransPortMeans.ArrivalConveyanceFacility.ID.c_str(),
														m_csm007.MessageBody.Declaration.Agent.ID.c_str(),
														m_csm007.MessageBody.Declaration.Agent.Contact.Name.c_str(),
														m_csm007.MessageBody.Declaration.IssueDate.c_str(),
														m_csm007.MessageBody.Declaration.AdditionalInformation.Content.c_str(),
														I_E_FLAG);
			...
			LOG1("insert into D_IM_FORECAST, MESSAGE_ID=%s", m_csm007.MessageHead.MessageID.c_str());
		}
...
void CCSM007::SendResponse()
{
	LOG3("Enter CCSM007::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_csm007.MessageHead.MessageType.c_str(), m_csm007.MessageHead.SenderID.c_str(), m_csm007.MessageHead.ReceiverID.c_str());
	if (m_csm007.MessageHead.MessageID.empty())
		return;
	...
	//	m_csm007.MessageHead.SenderID.c_str(),
	//	MESSAGE_CREATE_TIME,
	//	m_csm007.MessageHead.Version.c_str(),
	//	m_csm007.MessageHead.MessageID.c_str(),
	//	m_csm007.MessageBody.Declaration.BorderTransPortMeans.ID.c_str(),
	//	m_csm007.MessageBody.Declaration.BorderTransPortMeans.JourneyID.c_str(),
	//	RecType.c_str(),
...
int CCSM007::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='ALL' and INOROUTFLAG='0'", m_csm007.MessageHead.ReceiverID.c_str());
...
int CCSM007::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='WEB' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB' and INOROUTFLAG='0'", m_csm007.MessageHead.ReceiverID.c_str());
