...
CCSM004::CCSM004(void) : m_chOpFlag('F'), m_chSEND_FLAG('1'), m_strNote(""),m_csm004()
{
}

CCSM004::~CCSM004(void)
{
}

void CCSM004::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCSM004::SaveFileName(const std::string &strFileName)
{
...
//
//int CCSM004::SaveLocal(BYTE *pMessage, int iLen)
//{
...
void CCSM004::ParseMessage()
{
	try
	{
		...
		if (MessageIDHandle.Node() != NULL)	
			m_csm004.MessageHead.MessageID 			= MessageIDHandle.Text()->Value();
		if (FunctionCodeHandle.Node() != NULL)	
			m_csm004.MessageHead.FunctionCode 			= FunctionCodeHandle.Text()->Value();
		if (MessageTypeHandle.Node() != NULL)	
			m_csm004.MessageHead.MessageType 			= MessageTypeHandle.Text()->Value();
		if (SenderIDHandle.Node() != NULL)	
			m_csm004.MessageHead.SenderID 			= SenderIDHandle.Text()->Value();
		if (ReceiverIDHandle.Node() != NULL)	
			m_csm004.MessageHead.ReceiverID 			= ReceiverIDHandle.Text()->Value();
		if (SendTimeHandle.Node() != NULL)	
			m_csm004.MessageHead.SendTime 			= SendTimeHandle.Text()->Value();
		if (VersionHandle.Node() != NULL)	
			m_csm004.MessageHead.Version 			= VersionHandle.Text()->Value();

		if (ID.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.ID 			= ID.Text()->Value();
		if (Name.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.Name 			= Name.Text()->Value();
		if (JourneyID.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.JourneyID 					= JourneyID.Text()->Value();
		//if (JourneyID2.Node() != NULL)	
		m_csm004.MessageBody.Declaration.BorderTransPortMeans.JourneyID2 						= JourneyID2;

		if (ArrivalDateTime.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.Itinerary.ArrivalDateTime 					= ArrivalDateTime.Text()->Value();

		if (StayID.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.StayID 					= StayID.Text()->Value();

		if (DepartureConveyanceFacilityID.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.DepartureConveyanceFacility.ID 					= DepartureConveyanceFacilityID.Text()->Value();

		if (ArrivalConveyanceFacilityID.Node() != NULL)	
			m_csm004.MessageBody.Declaration.BorderTransPortMeans.ArrivalConveyanceFacility.ID					= ArrivalConveyanceFacilityID.Text()->Value();


		if (AgentID.Node() != NULL)	
			m_csm004.MessageBody.Declaration.Agent.ID 					= AgentID.Text()->Value();

		if (ContactName.Node() != NULL)	
			m_csm004.MessageBody.Declaration.Agent.Contact.Name					= ContactName.Text()->Value();

		if (IssueDate.Node() != NULL)	
			m_csm004.MessageBody.Declaration.IssueDate 					= IssueDate.Text()->Value();

		if (Content.Node() != NULL)	
			m_csm004.MessageBody.Declaration.AdditionalInformation.Content 					= Content.Text()->Value();
...
int CCSM004::SaveToDB()
{
	return 1;
	if (m_csm004.MessageHead.MessageID.empty())
		return 0;
	...
	try
	{
		...
		if (!m_csm004.MessageHead.MessageID.empty())
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_IM_ARRIVAL(MessageID,FunctionCode,MessageType,SenderID,ReceiverID,SendTime, \
														IMO, SHIP_NAME_EN, IM_VOYAGE_NO, EX_VOYAGE_NO, ARRIVE_TIME, \
														StayID, LOADING_PORT, DESTINATION_PORT, AGENT_CODE,\
														AGENT_NAME,ISSUE_TIME,CONTENT) \
														values('%s', '%s', '%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%s', \
														'%s', '%s', '%s')", \
														m_csm004.MessageHead.MessageID.c_str(),
														m_csm004.MessageHead.FunctionCode.c_str(),
														m_csm004.MessageHead.MessageType.c_str(),
														m_csm004.MessageHead.SenderID.c_str(),
														m_csm004.MessageHead.ReceiverID.c_str(),
														m_csm004.MessageHead.SendTime.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.ID.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.Name.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.JourneyID.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.JourneyID2.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.Itinerary.ArrivalDateTime.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.StayID.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.DepartureConveyanceFacility.ID.c_str(),
														m_csm004.MessageBody.Declaration.BorderTransPortMeans.ArrivalConveyanceFacility.ID.c_str(),
														m_csm004.MessageBody.Declaration.Agent.ID.c_str(),
														m_csm004.MessageBody.Declaration.Agent.Contact.Name.c_str(),
														m_csm004.MessageBody.Declaration.IssueDate.c_str(),
														m_csm004.MessageBody.Declaration.AdditionalInformation.Content.c_str()
														);
			...
			LOG1("insert into D_IM_FORECAST, MESSAGE_ID=%s", m_csm004.MessageHead.MessageID.c_str());
		}
...
void CCSM004::SendResponse()
{
	LOG3("Enter CCSM004::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_csm004.MessageHead.MessageType.c_str(), m_csm004.MessageHead.SenderID.c_str(), m_csm004.MessageHead.ReceiverID.c_str());
	if (m_csm004.MessageHead.MessageID.empty())
		return;
	...
	std::string szLabel			= CMessageType::CSM004;
	...
	//	m_csm004.MessageHead.SenderID.c_str(),
	//	MESSAGE_CREATE_TIME,
	//	m_csm004.MessageHead.Version.c_str(),
	//	m_csm004.MessageHead.MessageID.c_str(),
	//	m_csm004.MessageBody.Declaration.BorderTransPortMeans.ID.c_str(),
	//	m_csm004.MessageBody.Declaration.BorderTransPortMeans.JourneyID.c_str(),
	//	RecType.c_str(),
...
std::string CCSM004::GetFileName()
{
	return m_strFileName;
}

int CCSM004::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	if (m_csm004.MessageHead.MessageID.empty())
		return 0;
	...
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='PROXY' and INOROUTFLAG='0'", m_csm004.MessageHead.ReceiverID.c_str());
...
int CCSM004::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='WEB' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB' and INOROUTFLAG='0'", m_csm004.MessageHead.ReceiverID.c_str());
