...
//m_messageHead.MESSAGE_ID, OP_FLAG, m_messageHead.MESSAGE_CREATE_TIME, m_respondInfo.NOTE);
			...
			CString strSql = "";
			char szSqlBuffer[2048] = {0};
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "update D_CONTAUNLOAD_BILL set AUDIT_FLAG = '%c', AUDIT_DATE=GetDate(), AUDIT_NOTE='%s' where MESSAGE_ID='%s'", \
				OP_FLAG,
				m_respondInfo.NOTE,
			...
			LOG1("update D_CONTAUNLOAD_BILL, MESSAGE_ID=%s", m_messageHead.MESSAGE_ID);
		}
	}
	catch(...)
	{
...
void CCLM60013::UTF8ToGBK( char *&szOut )
{
