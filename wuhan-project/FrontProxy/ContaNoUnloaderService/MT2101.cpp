...
CMT2101::CMT2101(void) : m_chOpFlag('F'), m_chSEND_FLAG('1'), m_strNote(""),m_mt2101(),m_mtResponse(),m_nFlag(0)
{
}
...
{
	LOG3("Enter CMT2101::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_mt2101.Head.MessageType.c_str(), m_mt2101.Head.SenderID.c_str(), m_mt2101.Head.ReceiverID.c_str());
	...
	if (!m_mtResponse.Declaration.BorderTransportMeans.JourneyID.empty())
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			std::string szLabel			= CMessageType::MT2101;
			szLabel += "-";
			szLabel += m_mt2101.Head.MessageID;
...
int CMT2101::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		//sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='%s' and INOROUTFLAG=1", m_mt2101.Head.SenderID.c_str(), m_mt2101.Head.ReceiverID.c_str());
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where ENTERPRISE_CODE='%s' and INOROUTFLAG=1", m_mt2101.Head.ReceiverID.c_str());
