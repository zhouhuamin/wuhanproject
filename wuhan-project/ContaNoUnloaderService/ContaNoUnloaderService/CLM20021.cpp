		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("SHIP_INFO")) != NULL)
				{
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("CHANGE_SHIP_ID");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						CHANGE_SHIP_ID = pNodeTmp->ToElement()->GetText();
					...
					pTmp = szNote;
					memcpy(szNote, CHANGE_SHIP_NAME_CN.c_str(), CHANGE_SHIP_NAME_CN.size() < 256 ? CHANGE_SHIP_NAME_CN.size() : 255);
					UTF8ToGBK(pTmp);
					szNote[255] = '\0';
					CHANGE_SHIP_NAME_CN = szNote;
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("SUPERVISION_CODE");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						SUPERVISION_CODE = pNodeTmp->ToElement()->GetText();
					...
					memcpy(m_shipInfo.CHANGE_SHIP_ID, CHANGE_SHIP_ID.c_str(), CHANGE_SHIP_ID.size());

					memcpy(m_shipInfo.CHANGE_SHIP_NAME_EN, CHANGE_SHIP_NAME_EN.c_str(), CHANGE_SHIP_NAME_EN.size());
					memcpy(m_shipInfo.CHANGE_SHIP_NAME_CN, CHANGE_SHIP_NAME_CN.c_str(), CHANGE_SHIP_NAME_CN.size());
					memcpy(m_shipInfo.CHANGE_VOYAGE_NO, CHANGE_VOYAGE_NO.c_str(), CHANGE_VOYAGE_NO.size());
					...
					memcpy(m_shipInfo.SUPERVISION_CODE, SUPERVISION_CODE.c_str(), SUPERVISION_CODE.size());
				}
...
FUNCTION_CODE, MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME, SEND_FLAG, CHANGE_SHIP_NAME_EN,CHANGE_SHIP_NAME_CN,CHANGE_VOYAGE_NO,SUPERVISION_CODE,CHANGE_SHIP_ID) \
values('%s', '%s', '%c', '%s', '%s',\
'%s', '%s', '%s', '%s', '%s', '%s', \
'%s', '%s', '%s', '%s', '%s', GetDate(), '%c','%s','%s','%s','%s','%s')", \
m_messageHead.MESSAGE_ID, 
...
m_shipInfo.SUPERVISION_CODE,m_shipInfo.CHANGE_SHIP_ID
);
...
void CCLM20021::UTF8ToGBK( char *&szOut )
{
