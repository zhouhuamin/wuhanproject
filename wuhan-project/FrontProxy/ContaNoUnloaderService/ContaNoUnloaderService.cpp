// ContaNoUnloaderService.cpp : 定义控制台应用程序的入口点。
//

#include "stdafx.h"
#include "ContaNoUnloaderService.h"
...
#define SZAPPNAME            "FrontProxyService"
// internal name of the service
#define SZSERVICENAME        "FrontProxyService_Service"
// displayed name of the service
#define SZSERVICEDISPLAYNAME "FrontProxyService  Service"
// list of service dependencies - "dep1\0dep2\0\0"
#define SZDEPENDENCIES       ""
#define SZSERVICECOMMENT "前置机代理"
...
void ReadConfigInfo(struct structConfigInfo &configInfo)
{
	...
	TiXmlHandle BackerQueueIPHandle = docHandle.FirstChildElement("ConfigInfo").ChildElement("BackerQueueIP", 0).FirstChild();
	TiXmlHandle BackerQueueNameHandle = docHandle.FirstChildElement("ConfigInfo").ChildElement("BackerQueueName", 0).FirstChild();
	TiXmlHandle BackerQueueLabelHandle = docHandle.FirstChildElement("ConfigInfo").ChildElement("BackerQueueLabel", 0).FirstChild();
	...
	LOG3("%s, %s, %s", configInfo.strBackerQueueIP.c_str(), configInfo.strBackerQueueName.c_str(), configInfo.strBackerQueueLabel.c_str());
}
...
UINT __stdcall ProcessGUIThread(LPVOID pParam)
{
	...
	do
	{
		try
		{
			...
			apBasic->ProcessMessage(pConfigInfo, strFileName);
