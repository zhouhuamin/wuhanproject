...
{
	LOG3("Enter CMT3101::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_mt3101.Head.MessageType.c_str(), m_mt3101.Head.SenderID.c_str(), m_mt3101.Head.ReceiverID.c_str());
	...
	if (!m_mtResponse.Declaration.BorderTransportMeans.JourneyID.empty())
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			szLabel += "-";
			szLabel += m_mt3101.Head.MessageID;
...
int CMT3101::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where ENTERPRISE_CODE='%s' and INOROUTFLAG=1", m_mt3101.Head.ReceiverID.c_str());
