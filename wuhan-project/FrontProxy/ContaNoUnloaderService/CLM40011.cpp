...
CCLM40011::CCLM40011(void) : m_messageHead(), m_clm40011(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote("")
{
}

CCLM40011::~CCLM40011(void)
{
}

void CCLM40011::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM40011::SaveFileName(const std::string &strFileName)
{
	...
	try
	{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("CONTAINER_ARRIVE_INFO")) != NULL)
				{
					string CUSTOMS_CODE = "";
					string AREA_CODE = "";
					string VE_NAME = "";
					...
					string CONTA_NO   = "";
					string F_CONTA_NO = "";
					string F_CONTA_MODEL	= "";
					string F_CONTA_SIZE		= "";
					string B_CONTA_NO		= "";
					string B_CONTA_MODEL	= "";
					string B_CONTA_SIZE		= "";
					string NOTE = "";
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("AREA_CODE");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						AREA_CODE = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("VE_NAME");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						VE_NAME = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("ARRIVE_TIME");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						ARRIVE_TIME = pNodeTmp->ToElement()->GetText();
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("F_CONTA_NO");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						F_CONTA_NO = pNodeTmp->ToElement()->GetText();


					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("F_CONTA_MODEL");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						F_CONTA_MODEL = pNodeTmp->ToElement()->GetText();


					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("F_CONTA_SIZE");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						F_CONTA_SIZE = pNodeTmp->ToElement()->GetText();



					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("B_CONTA_NO");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						B_CONTA_NO = pNodeTmp->ToElement()->GetText();



					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("B_CONTA_MODEL");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						B_CONTA_MODEL = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("B_CONTA_SIZE");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						B_CONTA_SIZE = pNodeTmp->ToElement()->GetText();
					...
					//// 赋值
					//memcpy(m_clm40011.CUSTOMS_CODE, CUSTOMS_CODE.c_str(), sizeof(m_clm40011.CUSTOMS_CODE) - 1);
					//memcpy(m_clm40011.AREA_CODE, AREA_CODE.c_str(), sizeof(m_clm40011.AREA_CODE) - 1);
					//memcpy(m_clm40011.VE_NAME, VE_NAME.c_str(), sizeof(m_clm40011.VE_NAME) - 1);
					//memcpy(m_clm40011.ARRIVE_TIME, ARRIVE_TIME.c_str(), sizeof(m_clm40011.ARRIVE_TIME) - 1);
					//memcpy(m_clm40011.F_CONTA_MODEL, F_CONTA_MODEL.c_str(), sizeof(m_clm40011.F_CONTA_MODEL) - 1);
					//memcpy(m_clm40011.F_CONTA_SIZE, F_CONTA_SIZE.c_str(), sizeof(m_clm40011.F_CONTA_SIZE) - 1);

					//memcpy(m_clm40011.B_CONTA_NO, B_CONTA_NO.c_str(), sizeof(m_clm40011.B_CONTA_NO) - 1);
					//memcpy(m_clm40011.B_CONTA_MODEL, B_CONTA_MODEL.c_str(), sizeof(m_clm40011.B_CONTA_MODEL) - 1);
					//memcpy(m_clm40011.B_CONTA_SIZE, B_CONTA_SIZE.c_str(), sizeof(m_clm40011.B_CONTA_SIZE) - 1);
					//memcpy(m_clm40011.NOTE, NOTE.c_str(), sizeof(m_clm40011.NOTE) - 1);

					// 赋值
					memcpy(m_clm40011.CUSTOMS_CODE, CUSTOMS_CODE.c_str(), CUSTOMS_CODE.size());
					memcpy(m_clm40011.AREA_CODE, AREA_CODE.c_str(), AREA_CODE.size());
					memcpy(m_clm40011.VE_NAME, VE_NAME.c_str(), VE_NAME.size());
					memcpy(m_clm40011.ARRIVE_TIME, ARRIVE_TIME.c_str(), ARRIVE_TIME.size());
					memcpy(m_clm40011.F_CONTA_MODEL, F_CONTA_MODEL.c_str(), F_CONTA_MODEL.size());
					memcpy(m_clm40011.F_CONTA_SIZE, F_CONTA_SIZE.c_str(), F_CONTA_SIZE.size());

					memcpy(m_clm40011.B_CONTA_NO, B_CONTA_NO.c_str(), B_CONTA_NO.size());
					memcpy(m_clm40011.B_CONTA_MODEL, B_CONTA_MODEL.c_str(), B_CONTA_MODEL.size());
					memcpy(m_clm40011.B_CONTA_SIZE, B_CONTA_SIZE.c_str(), B_CONTA_SIZE.size());
				}
			}
		}
...
void CCLM40011::SendResponse()
{
	LOG3("Enter CCLM40011::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	...
	//	std::string strQueueName	= (LPCTSTR)queueNameArray[j];
	//	std::string strMQIp			= (LPCTSTR)mqIPArray[i];
	//	std::string szLabel			= CMessageType::CLM40011;
	//	std::auto_ptr<CMessageSender> apBasic(new CMSMQSender(strMQIp, strQueueName, szLabel, szLabel));
	//	apBasic->SendMessage(spXmlData.get(), iLen);
	//}
...
std::string CCLM40011::GetFileName()
{
	return m_strFileName;
}

int CCLM40011::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM40011::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
