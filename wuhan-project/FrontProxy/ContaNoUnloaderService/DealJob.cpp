...
void CDealJob::ProcessJob(JobItem *pJob, CDealJob* pServer)
{
	...
	else
	{
		...
		strMessageType = (LPCTSTR)strTypeTmp;
	}
	std::string strXmlData = (LPCTSTR)pTmpJob->m_Data;
	...
	requestVect.push_back("CLM20013");
	...
	requestVect.push_back("CLM90031");
	...
	else if (strMessageType == CMessageType::LHT10010)
	{
		std::auto_ptr<CCLMBasic> apBasic(new CLHT10010());
		apBasic->ProcessMessage(&m_config, strXmlData);
	}
