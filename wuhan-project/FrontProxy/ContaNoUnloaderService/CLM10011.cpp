		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("BILL_INFO")) != NULL)
				{
					...
					if (I_E_FLAG != "")
					{
						m_billInfo.I_E_FLAG[0] = I_E_FLAG[0];
					}
...
{
	LOG3("Enter CCLM10011::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return;
...
int CCLM10011::SelectMQ()
{
	...
	try
	{
		...
		if (stmt != NULL)
		{
			...
			{
				CString strText = "";
				strMQPath = strText = stmt->ValueString(0);        //得到列值
				// TRACE("%s\n", strText);
			}
		}
