...
void CCSM301::ParseMessage()
{
	try
	{
		...
		if (RecContentHandle.Node() != NULL)	
			m_csm301.MessageBody.Declaration.REC.RecContent 		= RecContentHandle.Text()->Value();
		...
		char szNote[256] = {0};
		char *pTmp = szNote;
		memcpy(szNote, m_csm301.MessageBody.Declaration.REC.RecContent.c_str(), m_csm301.MessageBody.Declaration.REC.RecContent.size() < 256 ? m_csm301.MessageBody.Declaration.REC.RecContent.size() : 255);
		UTF8ToGBK(pTmp);
		m_csm301.MessageBody.Declaration.REC.RecContent = string(szNote);
...
void CCSM301::SendResponse(const std::string &strPreFlag)
{
	//if (m_messageHead.MESSAGE_ID[0] == '\0')
	//	return;
...
int CCSM301::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		...
		CString strMQPath = "";
		char szSql[1024] = {0};
		sprintf_s(szSql, sizeof(szSql), "select MQPATH from WH_MQPATH_CONFIG where MQMESSAGETYPE='ALL' and CUSTOMS_CODE='%s' and ENTERPRISE_CODE='WEB' and INOROUTFLAG='1'", m_csm301.MessageHead.SenderID.c_str());
