...
CCLM20022::CCLM20022(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM20022::~CCLM20022(void)
{
}

void CCLM20022::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM20022::SaveFileName(const std::string &strFileName)
{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("RESPOND_INFO")) != NULL)
				{
					...
					// 赋值
					//memcpy(m_respondInfo.MESSAGE_ID, MESSAGE_ID.c_str(), sizeof(m_respondInfo.MESSAGE_ID) - 1);
					//memcpy(m_respondInfo.OP_FLAG, OP_FLAG.c_str(), sizeof(m_respondInfo.OP_FLAG) - 1);
					//memcpy(m_respondInfo.NOTE, NOTE.c_str(), sizeof(m_respondInfo.NOTE) - 1);

					// 赋值
					memcpy(m_respondInfo.MESSAGE_ID, MESSAGE_ID.c_str(), MESSAGE_ID.size());
					//memcpy(m_respondInfo.OP_FLAG, OP_FLAG.c_str(), OP_FLAG.size());
				}
			}
		}
...
void CCLM20022::SendResponse()
{
	LOG3("Enter CCLM20022::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	...
	for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
	{
		...
		std::string szLabel			= m_messageHead.MESSAGE_TYPE; // CMessageType::CLM20022;
		szLabel += "-";
...
std::string CCLM20022::GetFileName()
{
	return m_strFileName;
}

int CCLM20022::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM20022::SelectResponseInfo(CString &strMESSAGE_ID, CString &strOP_FLAG, CString &strNOTE)
{
...
int CCLM20022::InsertResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM20022::DeleteResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM20022::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
