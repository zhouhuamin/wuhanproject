...
CCLM90022::CCLM90022(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM90022::~CCLM90022(void)
{
}

void CCLM90022::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM90022::SaveFileName(const std::string &strFileName)
{
...
void CCLM90022::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM90022::SendResponse:%s", strPreFlag.c_str());
	// 海关平台返回的回执
	if (strPreFlag == "WEB")
	{
		...
		for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
		{
			...
			std::string szLabel			= CMessageType::CLM90022;
...
std::string CCLM90022::GetFileName()
{
	return m_strFileName;
}

int CCLM90022::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
int CCLM90022::SelectResponseInfo(CString &strMESSAGE_ID, CString &strOP_FLAG, CString &strNOTE)
{
...
int CCLM90022::InsertResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM90022::DeleteResponseInfo()
{
	if (m_respondInfo.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM90022::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
		CStringArray mqPathArray;
