...
void CGUITransaction::SendResponse(const std::string &strPreFlag)
{
	...
	GetXmlFromDB(strMessageType, strSender, spXmlData, nXmlLen);
	SaveLocalFile(spXmlData, nXmlLen);
...
int CGUITransaction::GetXmlFromDB(std::string &strMessageType,std::string &strSender,boost::shared_array<char> &spXmlData, int &nXmlLen)
{
	try
	{
		char szSql[512] = {0};
		sprintf_s(szSql, sizeof(szSql), "select top 1 GUID, MESSAGE_TYPE, SENDER, REPORT from R_APPLY_REPORT where DEL_FLAG = 0");
		CString strResult		= "";
		CString strConn			= "";
		CString strGUID			= "";
...
int CGUITransaction::SaveLocalFile(boost::shared_array<char> &spXmlData, int &nXmlLen)
{
	if (nXmlLen <= 1)
	{
		return 0;
