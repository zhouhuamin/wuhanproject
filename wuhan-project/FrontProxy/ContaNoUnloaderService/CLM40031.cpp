...
CCLM40031::CCLM40031(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)
{
}

CCLM40031::~CCLM40031(void)
{
}

void CCLM40031::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM40031::SaveFileName(const std::string &strFileName)
{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("BILL_INFO")) != NULL)
				{
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("MOVE_ARR_TIME");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						MOVE_ARR_TIME = pNodeTmp->ToElement()->GetText();
					...
					//memcpy(m_billInfo.MOVE_ARR_TIME, MOVE_ARR_TIME.c_str(), sizeof(m_billInfo.MOVE_ARR_TIME) - 1);
					...
					memcpy(m_billInfo.MOVE_ARR_TIME, MOVE_ARR_TIME.c_str(), MOVE_ARR_TIME.size());
				}
...
int CCLM40031::SaveToDB()
{
	...
	try
	{
		...
		if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_MOVE_ARR_BILL(MESSAGE_ID,CUSTOMS_CODE,I_E_FLAG,SHIP_NAME_EN,VOYAGE_NO,BILL_NO,NOTE, \
FUNCTION_CODE, MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME,MOVE_ARR_TIME) \
...
m_messageHead.FUNCTION_CODE, m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE, 
m_messageHead.MESSAGE_CREATE_TIME,
m_billInfo.MOVE_ARR_TIME
);
			...
			LOG1("insert into D_MOVE_ARR_BILL, MESSAGE_ID=%s", m_messageHead.MESSAGE_ID);
		}
		...
		for (size_t i = 0; i < nContaSize; ++i)
		{
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_MOVE_ARR_CONTA(MESSAGE_ID,CONTA_NO,CONTA_MODEL,CONTA_SIZE,NOTE,BILL_NO,INPUT_TIME) \
...
void CCLM40031::SendResponse()
{
	LOG3("Enter CCLM40031::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return;
	...
	std::string MESSAGE_TYPE = CMessageType::CLM40032;
...
m_messageHead.SENDER, MESSAGE_CREATE_TIME, m_messageHead.VERSION, m_messageHead.MESSAGE_ID, m_chOpFlag, m_strNote.c_str());
	...
	// 发送MSMQ
	//CMSMQApiWrapper ob;
	std::string szLabel = CMessageType::CLM40032;
	...
	//	std::string strQueueName	= (LPCTSTR)queueNameArray[j];
	//	std::string strMQIp			= (LPCTSTR)mqIPArray[i];
	//	std::string szLabel			= CMessageType::CLM40031;
	//	std::auto_ptr<CMessageSender> apBasic(new CMSMQSender(strMQIp, strQueueName, szLabel, szLabel));
	//	apBasic->SendMessage(spXmlData.get(), iLen);
	//}
...
std::string CCLM40031::GetFileName()
{
	return m_strFileName;
}

int CCLM40031::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
...
int CCLM40031::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
