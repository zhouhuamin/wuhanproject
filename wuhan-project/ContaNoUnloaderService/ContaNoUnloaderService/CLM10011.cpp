...
CCLM10011::CCLM10011(void) : m_messageHead(), m_billInfo(), m_respondInfo(), m_chOpFlag('F'), \
							m_chSEND_FLAG('1'), m_strNote(""),m_contaInfoVect(0)

{
}

CCLM10011::~CCLM10011(void)
{
}

void CCLM10011::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM10011::SaveFileName(const std::string &strFileName)
{
		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("BILL_INFO")) != NULL)
				{
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("PX_FLAG");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						PX_FLAG = pNodeTmp->ToElement()->GetText();
					...
					memcpy(m_billInfo.PX_FLAG, PX_FLAG.c_str(), PX_FLAG.size());
...
int CCLM10011::SaveToDB()
{
	LOG("Enter CCLM10011::SaveToDB");
	...
	try
	{
		...
		if (m_messageHead.MESSAGE_ID[0] != '\0')
		{
			...
			char PX_FLAG = ' ';
			if (m_billInfo.PX_FLAG[0] != '\0')
				PX_FLAG = m_billInfo.PX_FLAG[0];
			...
			sprintf_s(szSqlBuffer, sizeof(szSqlBuffer), "insert into D_RELEASE_DICTATE_BILL(MESSAGE_ID,CUSTOMS_CODE,I_E_FLAG,SHIP_NAME_EN,VOYAGE_NO,BILL_NO,NOTE, \
														FUNCTION_CODE, MESSAGE_TYPE, SENDER, RECEIVER, CREATE_TIME, INPUT_TIME, VERSION,PX_FLAG, ENTRY_ID) \
														values('%s', '%s', '%c', '%s', '%s', '%s', '%s', \
														'%s', '%s', '%s', '%s', '%s', GetDate(), '%s', '%c', '%s')", \
														...
														m_messageHead.FUNCTION_CODE, m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE, m_messageHead.MESSAGE_CREATE_TIME, m_messageHead.VERSION, PX_FLAG, m_billInfo.ENTRY_ID);
...
void CCLM10011::SendResponse()
{
	...
	std::string MESSAGE_TYPE = CMessageType::CLM10012;
...
m_messageHead.SENDER, MESSAGE_CREATE_TIME, m_messageHead.VERSION, m_messageHead.MESSAGE_ID, m_chOpFlag, m_strNote.c_str());
	...
	// 发送MSMQ
	//CMSMQApiWrapper ob;
	std::string szLabel = CMessageType::CLM10012;
...
std::string CCLM10011::GetFileName()
{
	return m_strFileName;
}

int CCLM10011::SelectMQ()
{
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return 0;
...
void CCLM10011::UTF8ToGBK( char *&szOut )
{
