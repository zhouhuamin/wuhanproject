...
protected:
	int		GetXmlFromDB(const std::string &strParaGUID, std::string &strMessageType,std::string &strSender, boost::shared_array<char> &spXmlData, int &nXmlLen, const std::string &strTime);
	...
	int     UpdateDB(const std::string &strGUID, const std::string &strTime);

	int		GetGUIDFromDB(std::vector<structDataItem> &guidVect);
