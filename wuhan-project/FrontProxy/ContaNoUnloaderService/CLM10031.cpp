		...
		if(xmlRootElement)
		{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("BILL_INFO")) != NULL)
				{
					...
					char szNote[256] = {0};
					char *pTmp = szNote;
					memcpy(szNote, NOTE.c_str(), NOTE.size());
					UTF8ToGBK(pTmp);
...
{
	LOG3("Enter CCLM10031::SendResponse, MessageType:%s, Sender:%s, Receiver:%s", m_messageHead.MESSAGE_TYPE, m_messageHead.SENDER, m_messageHead.RECEIVE);
	if (m_messageHead.MESSAGE_ID[0] == '\0')
		return;
