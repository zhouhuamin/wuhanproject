...
CCLM50033::CCLM50033(void) : m_messageHead(), m_clm50033(), m_respondInfo(), m_chOpFlag('F'), \
m_chSEND_FLAG('1'), m_strNote("")
{
}

CCLM50033::~CCLM50033(void)
{
}

void CCLM50033::InitConfigInfo(structConfigInfo *pConfig)
{
...
void CCLM50033::SaveFileName(const std::string &strFileName)
{
			...
			if ((pNode = xmlRootElement->FirstChild("MESSAGE_LIST")) != NULL)
			{
				...
				if ((pNode = xmlListElement->FirstChild("RESPOND_INFO")) != NULL)
				{
					...
					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("MESSAGE_ID");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						m_clm50033.MESSAGE_ID = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("OP_FLAG");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						m_clm50033.OP_FLAG = pNodeTmp->ToElement()->GetText();

					if (xmlSubElement != NULL)
						pNodeTmp = xmlSubElement->FirstChildElement("NOTE");
					if (pNodeTmp != NULL && pNodeTmp->ToElement() != NULL && pNodeTmp->ToElement()->GetText() != 0)
						m_clm50033.NOTE = pNodeTmp->ToElement()->GetText();
				}
			}
		}
...
void CCLM50033::SendResponse(const std::string &strPreFlag)
{
	LOG1("CCLM50033::SendResponse:%s", strPreFlag.c_str());
	//// WEB应用发送
	//if (strPreFlag == "WEB")
	//{
	...
	for (int i = 0, j = 0; i < queueNameArray.GetSize() && j < mqIPArray.GetSize(); ++i,++j)
	{
		...
		std::string strQueueName	= (LPCTSTR)queueNameArray[j];
		std::string strMQIp			= (LPCTSTR)mqIPArray[i];
		std::string szLabel			= CMessageType::CLM50033;
		std::auto_ptr<CMessageSender> apBasic(new CMSMQSender(strMQIp, strQueueName, szLabel, szLabel));
		apBasic->SendMessage(spXmlData.get(), iLen);
	}
...
std::string CCLM50033::GetFileName()
{
	return m_strFileName;
}

int CCLM50033::SelectMQ(CStringArray &queueNameArray, CStringArray &mqIPArray)
{
	try
	{
...
//int CCLM50033::SelectMQForWeb(CStringArray &queueNameArray, CStringArray &mqIPArray)
//{
//	try
//	{
//		CStringArray mqPathArray;
